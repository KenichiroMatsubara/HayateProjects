...
export interface BootTorimiHostOptions {
...
async function defaultFetchBundle(bundleUrl: string): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`Torimi: バンドル取得に失敗しました（${res.status} ${bundleUrl}）`);
  }
  return res.text();
}
...
export async function bootTorimiHost(options: BootTorimiHostOptions): Promise<WebHost> {
  ...
  mount(host);
  // 確立した host を返す。full reload ループ（`startTorimiHost`）が次の reload で
...
/**
 * 明示エラー UI を載せる要素の id。`startTorimiHost` を呼ぶだけで、消費側が何も実装しなくても
 * この id の要素が保証される。e2e / 消費側は可視性と本文をこの id で検証・スタイル調整できる。
 */
export const TORIMI_ERROR_PANEL_ID = 'torimi-error';
...
function renderBuiltinErrorPanel(message: string): void {
  ...
  if (!panel) {
    ...
    panel.id = TORIMI_ERROR_PANEL_ID;
...
function clearBuiltinErrorPanel(): void {
  if (typeof document === 'undefined') return;
  document.getElementById(TORIMI_ERROR_PANEL_ID)?.remove();
}
...
/**
 * mount 後にアプリが投げた未捕捉例外 / 未処理の Promise 拒否を既定で画面に出す。`startTorimiHost`
...
export interface StartTorimiHostOptions {
  ...
  // ── テスト注入 seam ──────────────────────────────────────────────────────
  /**
   * 1 回分の fetch → eval → host → mount を行う seam。既定は {@link bootTorimiHost}。
   * 確立した {@link WebHost} を返し、次の reload でその `detach`（ADR-0124）を畳めるようにする。
   */
  readonly boot?: (options: BootTorimiHostOptions) => Promise<WebHost>;
  /** reload 購読を張る seam。既定は {@link subscribeReload}。 */
  readonly subscribe?: (options: SubscribeReloadOptions) => ReloadSubscription;
}

export interface TorimiHostHandle {
  /** ホストを止める：reload 購読を解除する。 */
  close(): void;
}

/**
 * Torimi Web ホストの full reload ループを起動する。初回に一度 boot（fetch → eval →
 ...
 */
export function startTorimiHost(options: StartTorimiHostOptions): TorimiHostHandle {
  const boot = options.boot ?? bootTorimiHost;
