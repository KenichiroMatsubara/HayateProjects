...
The Torimi Web host: it fetches and evals the App Bundle from the dev-server URL, then establishes the host bootstrap via `@hayate/host` (`createHayateWebHost`) and hands it to the bundle's mount. It speaks the same dev-server and mount contracts as the native host but consumes the web App Bundle. It carries no framework and no `@tsubame/renderer-hayate` of its own.
