 ...
 *
 * Demo Endpoint のデプロイは Play リリースと lockstep — 起点は `torimi-android-v*` タグと
...
const WORKFLOW_PATH = '.github/workflows/torimi-release.yml';
/** リリースタグの名前パターン（ADR-0003）。AAB ビルドも将来同じタグに乗る。 */
const RELEASE_TAG_PATTERN = 'torimi-android-v*';
...
describe('release lockstep workflow (ADR-0003)', () => {
  ...
  it('documents the required Cloudflare secrets and how to register them in the demo-endpoint README', () => {
    ...
    const readme = readFileSync(join(repoRoot, 'Torimi/demo-endpoint/README.md'), 'utf8');
