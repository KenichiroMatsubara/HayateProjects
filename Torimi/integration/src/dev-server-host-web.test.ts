...
describe('Dev Server ↔ Host wired together over a real network path', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'torimi-integration-'));
  ...
  afterEach(async () => {
    ...
    delete (globalThis as Record<string, unknown>)[TORIMI_PROTOCOL_VERSION_GLOBAL];
  });

  it('boots by fetching the real App Bundle from a real dev-server over HTTP', async () => {
    await writeFile(
      bundlePath,
      `globalThis.${TORIMI_PROTOCOL_VERSION_GLOBAL} = 7;
       globalThis.${TORIMI_MOUNT_GLOBAL} = (host) => { globalThis.__integrationMountedWith = host; };`,
    );
  ...
  it('rejects with ProtocolMismatchError when the served bundle is on a different protocol version', async () => {
    await writeFile(
      bundlePath,
      `globalThis.${TORIMI_PROTOCOL_VERSION_GLOBAL} = 1;
       globalThis.${TORIMI_MOUNT_GLOBAL} = () => {};`,
    );
    server = createBundleDevServer({ bundlePath });
    origin = await server.listen();
  ...
  it('delivers a real WebSocket reload broadcast to subscribeReload when the served bundle changes', async () => {
    ...
    await writeFile(bundlePath, 'globalThis.__torimiMount = () => { /* edited */ };\n');
