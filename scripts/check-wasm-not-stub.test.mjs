...
test('assessTarget fails a target with no built .wasm (the fresh-clone stub state)', () => {
  const v = assessTarget({ npmName: '@torimi/hayate-adapter-web', wasmExists: false, jsContent: JS_STUB });
...
test('assessTarget fails a target whose JS still carries the bootstrap stub marker', () => {
  ...
  const v = assessTarget({ npmName: '@torimi/hayate-adapter-web', wasmExists: true, jsContent: JS_STUB });
  assert.equal(v.ok, false);
...
test('assessTarget passes a real built target (wasm present, no stub marker)', () => {
  ...
  const v = assessTarget({ npmName: '@torimi/hayate-adapter-web', wasmExists: true, jsContent: realJs });
