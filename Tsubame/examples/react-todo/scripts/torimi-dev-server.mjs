// Torimi 最小 dev server の起動ラッパ（e2e / ローカル用・#531）。
//
...
// `playwright.config.ts` の TORIMI_DEV_PORT と一致させる既定ポート（solid 版 5181 と衝突回避）。
const DEFAULT_PORT = 5183;

const exampleRoot = fileURLToPath(new URL('..', import.meta.url));
const configFile = fileURLToPath(new URL('../vite.config.torimi.ts', import.meta.url));
