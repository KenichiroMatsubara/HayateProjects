...
/**
 * #533：Android（native, ADR-0112）バンドルが Torimi の protocol version 受け渡し契約を
 * 満たすことのガード。Web の `main.torimi.tsx`（#530/#531）と対称に、native バンドルも
 * eval 時に encoder の wire 版数を `__torimiProtocolVersion` へ立て、ネイティブホスト
 ...
 */
describe('Android App Bundle embeds the Torimi protocol version (#533)', () => {
  afterEach(() => {
    delete (globalThis as Record<string, unknown>).__torimiProtocolVersion;
  });
