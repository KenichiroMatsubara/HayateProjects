...
const SELF_SIGNATURE = 'miharashi-android-dev-server.mjs';
...
function reclaimStalePort(targetPort) {
  ...
  for (const pid of pids) {
    ...
    try {
      ...
      console.log(
        `Miharashi android dev: ポート ${targetPort} を掴んでいた前回の残留プロセス（PID ${pid}）を終了しました。`,
      );
    } catch {
      // 既に終了済みなど — 無視
    }
...
try {
  ...
  console.error(`Miharashi android dev: ポート ${port} で listen できませんでした（${err?.code ?? err}）。`);
  console.error('  別プロセスがこのポートを使っていないか確認してください（MIHARASHI_DEV_PORT で変更可）。');
  process.exit(1);
}
