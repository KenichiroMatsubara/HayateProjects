...
// native pump）はこのパッケージに無い。Hayate 側（`@torimi/hayate-host`）または App（合成ルート）
...
export { OP, ELEMENT_KIND } from '@torimi/tsubame-protocol-generated/protocol';
// wire の protocol version（バンドル encoder ↔ ホスト decoder の整合トークン）。App Bundle は内包する
// renderer-hayate のこの版数を Torimi 起動時の突き合わせ用に埋める（#530 / CONTEXT「Protocol Version」）。
export { PROTOCOL_VERSION } from '@torimi/tsubame-protocol-generated/protocol';
