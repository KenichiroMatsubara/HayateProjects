...
```ts
// @torimi/tsubame-app —— @torimi/tsubame-renderer-protocol だけに依存（Hayate ランタイム盲目）
interface Host {
...
5. **Host adapter の置き場所は段階的**。`@torimi/tsubame-app`（純粋）にも `@torimi/hayate-host`（Hayate→Tsubame は永久に依存なし）にも置けないため、host adapter は App 階層のグルー。各 5 行程度なので**当面は各 example に inline**。solid-demo と react-demo で `webHost` が同一になった時点＝2 つ目の adapter＝そこで初めて中立 App 階層パッケージへ抽出する（「1 adapter は仮の seam、2 adapter は本物の seam」）。
...
- **単一 `@torimi/tsubame-app` が `@torimi/hayate-host` を直接 import**: 利用は最も簡単だが `@tsubame/*` パッケージが Hayate ランタイムに依存し CONTEXT 境界・ADR-0004 を破る。→ 却下。純粋 orchestrator + Host adapter 別置きを採る。
...
- `examples/solid-demo`（solid）の `compose.ts` / `detect-mode.ts` / `main.tsx` 分岐と、`examples/react-demo` の DOM 固定は、`@torimi/tsubame-app` の `runTsubameApp` ＋ inline Host adapter ＋ FW ごとの mount へ移行する（実装は別 PR）。
