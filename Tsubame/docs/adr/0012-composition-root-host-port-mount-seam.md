...
- `examples/todo`（solid）の `compose.ts` / `detect-mode.ts` / `main.tsx` 分岐と、`examples/react-todo` の DOM 固定は、`@torimi/tsubame-app` の `runTsubameApp` ＋ inline Host adapter ＋ FW ごとの mount へ移行する（実装は別 PR）。
