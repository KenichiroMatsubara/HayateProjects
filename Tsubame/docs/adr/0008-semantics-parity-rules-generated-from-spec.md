...
- **prop-op の適用**は、`setProperty` と同様に Renderer Protocol 独自 surface（Contract 外）として `renderer-protocol` パッケージに**共有 visitor** を置き、両 adapter が「DOM 書き込み」「Canvas enqueue」という**効果ハンドラ**だけを埋める。規則（どの prop-op がどの意味か）は 1 箇所。

規則は spec 正本、適用ロジックは protocol、効果（DOM / Canvas）のみ adapter——という分担にする。
...
- **protocol パッケージに手書き集約**：生成せず共有関数として置く。spec 正本と乖離しうる利点を捨てる。却下。
