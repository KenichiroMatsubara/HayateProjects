...
Tsubame と Hayate の結合点は `apply_mutations(ops: Float64Array, styles: Float32Array, texts: string[])` と `poll_events` 契約である。定数・opcode・style tag・event field 名の正本は Hayate の [`proto/spec/`](../../Hayate/proto/spec/)（`@torimi/hayate-protocol-spec`）である。
...
JS 内でフレーム分の変更を積み、`requestAnimationFrame` ごとに `apply_mutations` を 1回呼ぶ。`ops` と `styles` の意味論は `@torimi/hayate-protocol-spec` に従う。
