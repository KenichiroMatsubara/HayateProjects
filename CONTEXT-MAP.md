...
- **Torimi → Hayate ランタイム ＋ Tsubame.** 以下は Bundle Preview（Preview Mode の一方）の依存関係。もう一方の Live Preview は dev server 上の任意 Web アプリを WebView で実行するだけで、Tsubame にも App Bundle にも依存しない（Torimi ADR-0007）。Torimi は App（合成ルート）の一種で、Hayate のネイティブ runtime（ADR-0112 の Hermes 埋め込み＋`RawHayate` ブリッジ）に依存し、Tsubame Adapter ＋ `@tsubame/renderer-canvas` を内包した App Bundle をネットワーク経由で流し込んで実行する。ホストはフレームワークも renderer-canvas も持たない（バンドル側が持ち込む）。
