...
Hayate の公開境界として**かつて**使っていた設計語彙。当時は Element Layer と Raw Layer の二層を WIT で公開する構想だった。ADR-0049 で WIT 機構そのものを廃止し、現行の Hayate–Tsubame プロトコル正本は `proto/spec/*.json`（`apply_mutations` / `poll_events`、ADR-0049/0053）に移行済み。Raw Layer の外部公開も ADR-0072 で棄却（→ 上記「Raw Layer」参照）。現行文書で WIT に言及する場合は過去設計であることを明示する。一方向依存（Hayate は上位層を知らない）の原則自体は現行 proto 契約でも維持。Hayabusa は Rust クレートとして hayate-core に直接依存し、この公開境界を経由しない（ADR-0045）。
_Avoid_: 現行の公開 API・現行プロトコル正本（現行は `proto/spec/*.json`。WIT は歴史用語）
...
**Scroll Offset**:
`scroll-view` Element のスクロール位置（x, y）。`Element Document Runtime`（hayate-core）が基本 wheel 入力から offset を更新・保持する（nearest `scroll-view` 探索・content bounds クランプ込み、ADR-0053）。Hayate は `scene_build` 時に offset 分だけ子を平行移動しクリップを適用する。`position: sticky` も同 offset を使って `scene_build` 内でクランプする。イナーシャ・スナップ・rubber-band 等の物理演算は **Platform Adapter** が担い（ADR-0046）、raw 入力を変換して runtime の scroll API を呼ぶ。`scroll` イベントはアプリ通知専用で offset 積算には使わない。
_Avoid_: Hayate が scroll 状態を一切持たない設計（旧 ADR-0022）、物理演算を上位層 Hayabusa/Framework が持つ設計（旧 ADR-0022。現行は Platform Adapter）、StyleProp::ScrollOffset
