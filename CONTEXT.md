...
スクロール物理（フリング減衰曲線・overscroll の挙動・spring back）の「感触」を選ぶ閉じた語彙。`auto` / `ios` / `android` の三値で、`auto` は Platform Adapter が渡す platform 識別から各 OS 相当の感触へ解決する（完成形）。iOS 風（指数減衰＋sigmoid rubber-band）と Android 風（OverScroller の spline＋Material stretch）は別アルゴリズムで、いずれも Hayate Core が所有する。**Android の Material stretch overscroll は一様スケール近似で部分実装済み**（ADR-0131。限界端をビューポート境界にピン留めしたまま `1 + clamp(|overscroll|/dimension,0,1)*STRETCH_MAX` の一様スケールでコンテンツを内側へ伸ばし、scene lowering の scroll group アフィン一箇所に閉じる。物理・保存 offset・イベントは iOS とパリティ）。dev `tuning.json` の `profile: "android"` でのみ選択でき、`auto`（UA 自動解決）と Android の spline fling・非線形フォールオフ歪みは将来。
