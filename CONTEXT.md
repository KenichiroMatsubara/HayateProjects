...
**引き継ぎプロンプト・コピペ用プロンプトを求められたとき:**
ファイルに書き出さない。チャットに直接出力する。プロンプト全体を1つのコードブロック（```）で囲む。コードブロックの中にコードブロックを入れるとコピーボタンが壊れるため、コード例はインデントのみで記述し ``` を使わない。これによりモバイルのコピーボタン1タップで全文コピーできる。
