...
**実装済み（2026-06-24・ADR-0010）**：
- **オーサリング**：要素インライン `style="k: v; ..."`（`<style>` ブロック＋セレクタ・scoped は後続）。
- **語彙（閉じた部分集合）**：`width`/`height`/`padding`/`margin`/`gap`/`display`/`flex-direction`/
  `align-items`/`justify-content`／`background-color`/`color`/`font-size`。単位 `px`/`%`/`auto`、
  色 `#hex`＋名前付き少数。Hayabusa `style::StyleProp`（閉じた型）→ `HayateSink` が core へ写す。
- **sink op**：`set_style` ＋ `Mutation::SetStyle`。instantiate 時に一度だけ適用（reactive 禁止）。
- **codegen**：`.hybs` の `style="..."` を型検査される `StyleProp::...` へ。`components/counter.hybs` を styled に。
- 検証：`tests/style.rs`（一度だけ・signal 非反応）/ `tests/app_host.rs`（実 layout 読み戻し）/ codegen unit。
- 詳細は [ADR-0010](adr/0010-static-style-dsl-and-set-style-op.md)。reactive style・`<style>` セレクタ・
  scoped style・`:hover` 等は後続の別 ADR。
...
**実装済み（2026-06-23・ADR-0007）**：
- **ガード（core）**：`ElementTree::element_set_text_content_if_idle`（`text_content` と差分があり、
  かつ `preedit` 無し＝非組成中のときだけ適用）。core `value_guard_tests` で検証。
- **sink op**：`ElementSink::set_value` ＋ `Mutation::SetValue`。`HayateSink`／`apply_mutation` は
  上記 core ガードへ写す。`RecordingSink` はガード無しで記録（配線観測用）。
- **Template IR**：`TemplateNode::bind_value`（programmatic value 束縛・signal 変化で `set_value`）と
  `on_input`（commit テキストを payload で受ける）。`on:click` と handler 列を共有。
- **配送**：`HayabusaApp` が `DocumentEventKind::TextInput` の listener を登録し、`Event::TextInput`
  を `Instance::input(elid, text)` へルーティング（読み・主）。`value` 束縛 Effect が `set_value` を
  発行（書き・従、ガードで echo 抑止）。
- **codegen**：`.hybs` の `on:input={h}` / `value={expr}` を解釈。`components/text_field.hybs` がデモ。
- 検証：`tests/input_binding.rs`（配線・RecordingSink）/ core `value_guard_tests`（ガード）/
  `tests/app_host.rs`（実 `EditState` 越しの読み・書き・clear）/ `tests/hybs_codegen.rs`（生成）。

**残る後続**：`on:submit` 等の form レベルイベント、IME composition イベントの Hayabusa 側配送。
...
3. ~~**P6 `.hybs` codegen**（ADR-0008・build 時 codegen）~~ ✅ **完了（ADR-0008）**：
   ...
   `:if`/`:each`・`<style>`（P3 待ち）。
4. ~~**P4 入力束縛**（ADR-0007・EditState 単一正本）~~ ✅ **完了（ADR-0007）**：sink の
   `set_value`（core 差分・非組成中ガード）＋ `bind_value`／`on:input`＋ `TextInput` ルーティング＋
