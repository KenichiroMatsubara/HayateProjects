...
## P4 🟠 フォーム／`text-input` の双方向束縛 — **意味論を決定（要 ADR 化）**
...
>
> **要 ADR 化**：本決定は「controlled だが signal は編集中の正本ではない」という非自明な非対称で、
> 後から覆すと入力束縛 API が変わるため ADR に残す（候補名：「入力 / value 束縛の意味論 —
> EditState 単一正本」）。sink/IR には programmatic な value set オペ（差分・非組成中ガード付き）が要る。
