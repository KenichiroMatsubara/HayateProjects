...
## 実装（2026-06-23・tracer bullet）

- **`hayabusa-codegen` クレート**（`codegen/`）を `[build-dependencies]` に置いた（クレートは
  自身のライブラリを build.rs から使えないため）。`compile_hybs(source, name)` が section 分割→
  `<template>` markup パース→生成 Rust を返す。
- **`build.rs`** が `components/*.hybs` を codegen して `$OUT_DIR` へ出し、`src/lib.rs` の
  `pub mod generated` が `include!` する。各コンポーネントは `generated::<name>::build(rt, sink)`。
- **名前の配線**：`{expr}` の自由変数（根の識別子）→ `Scope::with(.., Binding::Signal(v.clone()))`、
  `on:click={h}` → `Vec<Handler>`（出現順・重複除去）へ。`<script>` の Rust は build 関数本体へ
  verbatim 差し込み（cargo が型検査）。よって `<script>` は handler が捕捉する signal を内部で
  clone し、束縛参照する signal を move し切らない規約を取る（生成された scope 配線が `v.clone()`
  を読むため）。
- **範囲**：要素ネスト・静的テキスト・`{expr}` 単体束縛・`on:click`。`:if`/`:each`/子コンポーネント・
  mixed text・複数 `{expr}`・`<style>` の static style 生成（P3 の `set_style` 待ち）は後続で、
  パーサは未対応構文を明示的にエラーにする。`components/counter.hybs` ＋ `tests/hybs_codegen.rs`
  （手組み counter と同一挙動）/ `tests/app_host.rs`（`.hybs`→App Host→実 ElementTree）で実証。
