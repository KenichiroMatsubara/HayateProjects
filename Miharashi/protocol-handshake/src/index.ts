 ...
 * `__miharashiMount`（`@miharashi/host-web` の MIHARASHI_MOUNT_GLOBAL）と対称の、バンドル →
 * ホストの受け渡しシーム。global 名は wire 契約なので Web/Android で共有する定数に固定する。
 */
export const MIHARASHI_PROTOCOL_VERSION_GLOBAL = '__miharashiProtocolVersion';

/**
 * eval 済みバンドルが {@link MIHARASHI_PROTOCOL_VERSION_GLOBAL} に立てた protocol version を読む。
...
export function readBundleProtocolVersion(scope: object): number | undefined {
  const value = (scope as Record<string, unknown>)[MIHARASHI_PROTOCOL_VERSION_GLOBAL];
