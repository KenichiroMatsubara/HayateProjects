# @miharashi/demo-endpoint

Miharashi の **Demo Endpoint**（ADR-0003）。ビルド済みデモ App Bundle と Demo Manifest
...
```sh
pnpm --filter @miharashi/demo-endpoint run build:demos   # solid / react の build:android → public/
pnpm --filter @miharashi/demo-endpoint run dev           # wrangler dev
```
...
本番デプロイは **リリース lockstep CI**（`.github/workflows/miharashi-release.yml`）の領分。
`miharashi-android-v*` タグの push か手動実行（workflow_dispatch）だけがトリガで、main への
...
4. **動作確認** — Actions タブから `Miharashi release lockstep` を workflow_dispatch で手動実行し、
