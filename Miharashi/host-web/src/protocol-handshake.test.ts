...
import { ProtocolMismatchError } from '@miharashi/protocol-handshake';
import { bootMiharashiHost } from './index.js';
import type { WebHost } from '@hayate/host';

/**
 * Miharashi 起動時の protocol version ハンドシェイク契約（#530）。バンドル（encoder）が埋めた
...
describe('bootMiharashiHost — protocol version handshake', () => {
  ...
  it('不一致エラーは両バージョンを構造化して運ぶ（明示 UI 用）', async () => {
    const error = await bootMiharashiHost({
      devServerUrl: 'http://dev.example',
      canvas,
      hostProtocolVersion: 1,
      fetchBundle: async () => 'src',
