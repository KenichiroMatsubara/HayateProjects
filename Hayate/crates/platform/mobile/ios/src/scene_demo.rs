...
//!
//! ビルダーは `hayate-core` の要素 API のみを使う（objc2/UIKit 非依存）ため、ツリーの
//! 操作可能性をホスト上でテストできる継ぎ目になる。`app.rs` はビューポートをサイズ設定
//! して CADisplayLink ごとに描画する薄いグルー。`hayate-adapter-android` の `scene_demo`
//! と同型（共有 adapter-util crate は現状ないため対称性のためにミラーする）。
