//! Miharashi Android ホストの full reload ループ（#533）。
//!
//! Web ホスト（`@miharashi/host-web` の `bootMiharashiHost` / `subscribeReload` /
//! `startMiharashiHost`, #529）と対称の**ホスト側 full reload**。dev-server からの WS `reload`
...
/// dev-server が reload シグナルを流す WS ルート。`@miharashi/dev-server` の `RELOAD_ROUTE` /
...
/// Miharashi Android ホストの 1 boot：dev-server からバンドルを取得 → Hermes ランタイムを
...
/// 突き合わせ配線だけを担うのでホストで契約テストできる（Web の `bootMiharashiHost` と対称）。
#[cfg_attr(not(target_os = "android"), allow(dead_code))]
pub fn boot_runtime<R>(
    ...
    // ランタイム構築 = バンドルの eval（global `__miharashiProtocolVersion` / `__tsubame` が立つ）。
...
mod tests {
    ...
    fn boot_builds_the_runtime_and_returns_it_when_versions_match() {
        let runtime = boot_runtime(
            1,
            || Ok("globalThis.__miharashiProtocolVersion = 1;".to_owned()),
            |src| FakeRuntime { bundle: src.to_owned() },
            |_rt| Some(1),
        )
        .expect("matching versions should boot");
        assert_eq!(runtime.bundle, "globalThis.__miharashiProtocolVersion = 1;");
    }
