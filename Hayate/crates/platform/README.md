...
| **clipboard** | （capability 化しない） | ADR-0014 の Platform Adapter 責務として ADR-0097 が編集境界 `element::clipboard::Clipboard`（選択テキストのコピペ）で所有済み。同一 OS API への 2 重抽象を避けるため capability scaffold には含めない（ADR-0118）。 |
