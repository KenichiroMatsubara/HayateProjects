...
fn placed_item_rect(placement: Vec<StyleProp>) -> (f32, f32, f32, f32) {
    ...
    tree.element_layout_rect(item).expect("item must have layout")
}
...
fn grid_column_line_places_item_in_that_column() {
    ...
    assert!((x - TRACK).abs() < 1.0, "grid-column: 2 lands in column 2, x={x}");
    assert!((w - TRACK).abs() < 1.0, "single-line placement spans one track, w={w}");
}
...
fn grid_row_line_places_item_in_that_row() {
    ...
    assert!((h - TRACK).abs() < 1.0, "single-line placement spans one track, h={h}");
}
...
fn grid_column_span_occupies_multiple_tracks() {
    ...
    assert!((x - 0.0).abs() < 1.0, "auto-placed span starts at column 1, x={x}");
    assert!((w - 2.0 * TRACK).abs() < 1.0, "span 2 occupies two tracks, w={w}");
}
...
fn grid_column_start_end_spans_the_explicit_range() {
    ...
    assert!((x - TRACK).abs() < 1.0, "start line 2 begins at column 2, x={x}");
    assert!(
        (w - 2.0 * TRACK).abs() < 1.0,
