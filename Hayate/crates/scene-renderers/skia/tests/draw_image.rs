...
fn draw_image_scales_and_places_pixels() {
    ...
    assert!(
        top_left[0] > 200 && top_left[1] < 60,
        "top-left quadrant should be red, got {top_left:?}"
    );
    assert!(
        top_right[1] > 200 && top_right[0] < 60,
        "top-right quadrant should be green, got {top_right:?}"
    );
    assert!(
        bottom_left[2] > 200 && bottom_left[0] < 60,
        "bottom-left quadrant should be blue, got {bottom_left:?}"
    );
