...
use hayate_layer_compositor::{
    collect_layer_placements, extract_layer_scene, extract_root_scene, scroll_layer_geometry,
    scroll_layer_geometry_table, tunables, CompositeQuad, LayerCompositor, LayerRasterizer,
    PresentPlanner,
};
...
fn tall_list_tree() -> (ElementTree, ElementId) {
    ...
    tree.element_set_style(
        scroll,
        &[StyleProp::Width(px(W as f32)), StyleProp::Height(px(SCROLL_VIEWPORT_H))],
    );
    tree.element_set_style(
        content,
        &[
...
fn pump_scroll_layer(
    ...
    if !geometry.content_dirty
        && !planner.scroll_layer_needs_raster(scroll, geometry.visible_top, geometry.viewport_height)
    {
        return false;
    }
    let extracted = if scroll == root {
        ...
        extract_layer_scene(graph, scroll, &boundaries).expect("scroll view is lowered")
    };
    let raster_band = geometry.raster_band();
    rasterizer.rasterize(scroll, &extracted, Some(raster_band)).unwrap();
    let bytes = rasterizer.scroll_band_bytes(geometry.band);
...
fn rasterize_non_scroll_layers(tree: &ElementTree, scroll: ElementId, rasterizer: &mut VelloLayerRasterizer) {
    let graph = tree.scene_graph();
    let root = tree.frame_layers()[0];
    let boundaries: HashSet<ElementId> = tree.frame_layers().iter().copied().collect();
...
fn composite_frame(harness: &VelloHarness, tree: &ElementTree, rasterizer: &VelloLayerRasterizer, planner: &PresentPlanner) -> Vec<u8> {
    let graph = tree.scene_graph();
    let root = tree.frame_layers()[0];
    let boundaries: HashSet<ElementId> = tree.frame_layers().iter().copied().collect();
    ...
    let target_texture = harness.device.create_texture(&wgpu::TextureDescriptor {
        ...
        size: wgpu::Extent3d { width: W, height: SURFACE_H, depth_or_array_layers: 1 },
        mip_level_count: 1,
        sample_count: 1,
    ...
    let quads: Vec<CompositeQuad<'_, _>> = placements
        ...
        .filter_map(|placement| {
            rasterizer.texture(placement.layer).map(|texture| {
                ...
                let transform = match (planner.cached_scroll_band(placement.layer), scroll_geometry.get(&placement.layer)) {
    ...
    readback_rgba8(&harness.device, &harness.queue, &target_texture, W, SURFACE_H).expect("readback")
}
...
fn scroll_layer_texture_is_rastered_at_band_size_not_full_surface() {
    ...
    let mut rasterizer =
        VelloLayerRasterizer::new(harness.device.clone(), harness.queue.clone(), W, SURFACE_H, 1.0).unwrap();
    let rastered = pump_scroll_layer(&tree, scroll, &mut planner, &mut rasterizer);
    assert!(rastered, "cold frame must raster");

    let texture = rasterizer.texture(scroll).expect("scroll layer has a cached texture");
    ...
    assert_eq!(texture.width, W, "only the vertical axis is banded (ADR-0127 scope)");
    assert!(
    ...
    assert!(
        planner.cached_bytes() < full_surface_bytes,
        "budget must not charge full-surface bytes ({full_surface_bytes}) for a banded scroll layer (got {})",
        planner.cached_bytes()
    );
    assert_eq!(
        planner.cached_bytes(),
        u64::from(W) * u64::from(expected_band_height) * tunables::BYTES_PER_PIXEL,
        "charged bytes must equal width * band device height * bytes-per-pixel exactly"
    );
}
...
fn scrolling_within_the_cached_band_rasters_the_scroll_layer_zero_times() {
    ...
    let mut rasterizer =
        VelloLayerRasterizer::new(harness.device.clone(), harness.queue.clone(), W, SURFACE_H, 1.0).unwrap();
    assert!(pump_scroll_layer(&tree, scroll, &mut planner, &mut rasterizer), "cold frame rasters");
...
fn assert_banded_scroll_present_matches_full_raster(harness: &mut VelloHarness, tree: &ElementTree, label: &str) {
    let scroll = tree.frame_layers()[1];
    let full = render_scene_to_pixels_scaled(harness, tree.scene_graph(), W, SURFACE_H, 1.0).expect("full raster");

    let mut rasterizer =
        VelloLayerRasterizer::new(harness.device.clone(), harness.queue.clone(), W, SURFACE_H, 1.0).unwrap();
    let mut planner = PresentPlanner::new();
    rasterize_non_scroll_layers(tree, scroll, &mut rasterizer);
    assert!(pump_scroll_layer(tree, scroll, &mut planner, &mut rasterizer), "scroll layer must raster");
...
fn banded_present_matches_full_raster_scrolled_deep_into_a_long_list() {
    ...
    tree.element_set_scroll_offset(scroll, 0.0, 4000.0);
    let _ = tree.render(16.0);
    assert_banded_scroll_present_matches_full_raster(&mut harness, &tree, "scrolled deep into a long list");
}
...
fn banded_present_matches_full_raster_near_content_top() {
    ...
    let _ = tree.render(16.0);
    assert_banded_scroll_present_matches_full_raster(&mut harness, &tree, "near content top (clamped band)");
}
...
fn banded_present_tracks_further_in_band_scrolling_without_a_reraster() {
    ...
    let mut planner = PresentPlanner::new();
    rasterize_non_scroll_layers(&tree, scroll, &mut rasterizer);
    assert!(pump_scroll_layer(&tree, scroll, &mut planner, &mut rasterizer), "cold frame must raster");
    ...
    let full = render_scene_to_pixels_scaled(&mut harness, tree.scene_graph(), W, SURFACE_H, 1.0).expect("full raster");
    let layered = composite_frame(&harness, &tree, &rasterizer, &planner);
    assert_pixels_match(&full, &layered, "composite-only frame after scrolling further within the cached band");
}
