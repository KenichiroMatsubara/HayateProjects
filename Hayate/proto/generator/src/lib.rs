...
fn generate(proto: &Proto) -> String {
    ...
    for op in &proto.opcodes {
        out.push_str(&format!("pub const OP_{}: u32 = {};\n", op.name, op.value));
    }
    out.push('\n');
...
fn generate_style_codec(proto: &Proto) -> String {
    ...
    for en in &proto.enums {
        ...
        for v in &en.values {
            ...
            out.push_str(&format!("        {} => {rust_enum}::{variant},\n", v.value));
        }
...
fn generate_codec(proto: &Proto) -> String {
    ...
    for op in &proto.opcodes {
        ...
        if op.params.is_empty() {
            out.push_str(&format!(
                "        Op::{variant} => buf.push(OP_{} as f64),\n",
                op.name
            ));
            continue;
        }
        ...
        for p in &op.params {
            ...
            if count > 1 {
                ...
                let push = match p.typ.as_str() {
                    "element_id" | "u32" | "usize" => {
                        format!("buf.push(*{name} as f64);", name = p.name)
                    }
                    "f32" => format!("buf.push(*{name} as f64);", name = p.name),
                    "f64" => format!("buf.push(*{name});", name = p.name),
                    "bool" => format!(
                        "buf.push(if *{name} {{ 1.0 }} else {{ 0.0 }});",
                        name = p.name
                    ),
    ...
    for tag in &proto.style_tags {
        ...
        out.push_str(&format!(
            "            StyleProp::{prop_variant}{binding} => {{\n"
        ));
        out.push_str(&format!(
            "                buf.push(TAG_{} as f32);\n",
            tag.name
        ));
...
fn generate_dom_style_mapper(proto: &Proto) -> String {
    ...
    out.push_str("// ── Style Channel predicates (generated) ───────────────────────────────\n\n");
...
fn generate_dispatch(proto: &Proto) -> String {
    ...
    out.push_str("        _ => Err(format!(\"unknown unset style kind {v}\")),\n");
    out.push_str("    }\n");
    out.push_str("}\n\n");
...
fn encode_event_wire_param_push(param: &Param, field: &str) -> String {
    match param.typ.as_str() {
        ...
        "f32" | "f64" | "u32" | "usize" | "bool" => {
            format!(
                "            out.push(EventWireValue::Number(*{} as f64));\n",
                field
            )
        }
        "pointer_kind" => format!(
...
fn generate_element_kind_tables(proto: &Proto) -> String {
    ...
    out.push_str("/// UA default cursor for `kind` when no explicit `cursor` is set (ADR-0105).\n");
