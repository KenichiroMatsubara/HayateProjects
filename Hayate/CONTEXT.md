...
**Scene Renderer**:
`SceneGraph` を消費して描画結果を生成する実装単位。Web は Vello / tiny-skia、Native は skia-safe / Vello を候補とし、`SceneGraph` の walk は `hayate-core` の `render_scene_graph` が一度だけ行う。
...
異なる `Scene Renderer` が同じ `SceneGraph` と描画意味論に対して、共通の成功結果・入力検証・失敗分類を返すこと。Vello / tiny-skia（Web）と skia-safe（Native）の API・surface 固有の詳細や低レベル原因まで同一にすることは含まない。
