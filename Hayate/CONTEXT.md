...
**Interaction**:
hayate-core の interaction 状態機械を `ElementTree` 内に閉じ込めた deep module（ADR-0122）。横断的 interaction state（focus / hover / active / press 位置 / `PointerGesture` / `PointerKind` / `InputModality` / 直近 pointer 位置・cursor / touch scroll）を所有し、`ElementTree::apply_interaction_intent(InteractionIntent)` 単一 seam を公開する。private `InteractionSession` が tree の排他的借用内で state と element/layout/edit/scroll/event の document 能力をまとめ、全 arm を単一 match で適用する。`InteractionTreeView`、fake view、placeholder による一時移動は持たず、実 `ElementTree` が test surface になる。ADR-0066 が runtime 所有とした interaction 状態の所在をこの module へ精緻化する。
...
Hayate リポジトリの `proto/spec/` に置く、Hayate-Tsubame 間の機械可読な契約。`opcodes`・`style_tags`・`event_kinds`・`element_kinds`・`unset_kinds`・`modifier_keys` 等を JSON で定義し、`proto/spec/schema/` の JSON Schema で検証する。正本は Hayate 側のみ。Hayate は `proto/generator/` から wire 定数・decode・encode（`codec.rs`）を `proto/generated/` に生成し commit する。Tsubame は npm パッケージ `@torimi/hayate-protocol-spec` 経由で Contract を取り込み、`Tsubame/proto/generator/` から wire 定数・TS encode（`codec.ts`）・adapter vocabulary（`catalog.ts` 等）を `Tsubame/proto/generated/` に生成し commit する。`style_tags` の `encodeFrom` は TS 向け入力変換規則（ADR-0055）。`style_tags` の `domCss`（tag→ブラウザ CSS 写像・`DOM_EXTRAS` 含む）も spec 正本で、`dom_style_mapper.rs`（Rust HTML Mode）と `catalog.ts`（TS DOM Renderer）を両側生成する（ADR-0070）。Renderer Protocol 独自の surface（`setProperty`・`addEventListener` 購読 API）と HayateRenderer 所有の semantic mutation queue は Contract 外として手書きのまま残す。`resize` も Contract 外だが**もはや Renderer Protocol surface ではない** — viewport 追従は host→adapter→core（web は `hayate-adapter-web` の自己配線 ResizeObserver、android は native ループ）が `set_viewport` で所有し、Tsubame は resize 経路に存在しない（ADR-0080 を native へ延長, issue #475。当初の「resize は Renderer Protocol surface」記述の訂正）。
...
Tsubame `Hayate Renderer` と Hayate WASM の間で、フレーム内 mutation を `ops/styles/texts` の unified batch として発行する順序契約。HayateRenderer は semantic mutation の順序を保持し、flush 時に ADR-0052 の `apply_mutations` 境界へ一括エンコードする。
...
**SceneGraph**:
Hayate が保持する描画用の retained graph。element→scene lowering は dirty-gated incremental で、未 dirty の subtree はフレーム間で再利用し、commit は不変な `Scene Snapshot` を公開する（ADR-0086、ADR-0153）。UI フレームワークやコンポーネントツリーそのものではない。
_Avoid_: Virtual DOM, Component Tree

**Scene Snapshot（シーンスナップショット）**:
一回の Core commit で確定した不変な scene value。後続 frame の lowering から独立した寿命を持ち、thread 間で共有しても内容が変化しない（ADR-0153）。
_Avoid_: mutable SceneGraph の借用、frame ごとの全 node copy、renderer 固有scene

**Layer Scene（レイヤーシーン）**:
`Scene Snapshot`を特定layerのcanonicalな描画内容として読むzero-copy projection。子孫の別layerを除外し、合成時に適用する外側transform・clip・scroll affineを内容へ焼き込まない（ADR-0153）。
_Avoid_: 抽出・複製されたSceneGraph、borrowed subgraph、rendererごとのlayer walk規約

**Element Anchor（要素アンカー）**:
`NodeKind::ElementAnchor` — element ごとの構造専用 retained Node。詳細はルート `CONTEXT.md` の Rendering Terms。

**Scene Renderer**:
`Scene Snapshot`全体または`Layer Scene`を消費して描画結果を生成する実装単位。Web は Vello / tiny-skia、Native は skia-safe / Vello を候補とし、canonical walkはSceneGraph moduleが一度だけ行う。
_Avoid_: Backend, Driver, adapter 内での独自scene walk
...
_Avoid_: iOS を永久対象外とする理解、Web renderer を Native family に含める説明
...
**Font Instance（フォントインスタンス）**:
font bytes、face index、variation、synthesisの組をCoreがinternしたrenderer-neutralなimmutable value。stable generational identityを持ち、Scene Rendererは内容を再hashせず具体font resourceへ解決する（ADR-0155）。
_Avoid_: font bytesだけのidentity、renderer固有typeface、pointer addressによるidentity

**Text Run（テキストラン）**:
`Font Instance`、font size、glyphと位置、decorationをCoreがinternしたimmutable shaped value。raster結果が変わると新しいstable generational identityになり、Scene Snapshot間では共有できる（ADR-0155）。
_Avoid_: frameごとのglyph列clone、renderer側で再構築するcache key、生textと同一視

**Cross-Renderer Parity（クロスレンダラ・パリティ）**:
異なる `Scene Renderer` が同じ `Scene Snapshot` / `Layer Scene` と描画意味論に対して、共通の成功結果・入力検証・失敗分類を返すこと。Vello / tiny-skia（Web）と skia-safe（Native）の API・surface 固有の詳細や低レベル原因まで同一にすることは含まない。
...
**Render Host**:
surface 初期化、capability 判定、renderer 切替、資源寿命とbudget policyを担う描画オーケストレーション層。描画そのものと`Render Resource Residency`は`Scene Renderer`が担う。tree/loop/event drain は持たず、それらは上層の App Host が所有して Render Host を駆動する。Tsubame Hayate Renderer（wire 経路）向けの web/native bootstrap は JS パッケージ `@torimi/hayate-host` が体現する：`createHayateWebHost(canvas)` が WebGPU プローブ・`Renderer Selection Policy`・WASM ロード・surface 取得を行い `RawHayate`(+frame-clock) を返し、`./native` の `createHayateNativeHost(raw)` が注入 RawHayate を vsync pump に結線する。host bootstrap は Tsubame の renderer パッケージには置かない（#477、ADR-0155）。
_Avoid_: Platform Adapter, Backend, App Host（tree/loop/drain を持つ上層）との同一視、host bootstrap を Tsubame renderer パッケージに置く設計

**Render Resource Residency（描画資源レジデンシー）**:
選択済み`Scene Renderer` instanceがbudget policyの下で保持するrenderer固有資源の集合。CPU-backed resourceとGPU resourceを別poolとして扱い、rendererとRaster Threadの寿命に揃える（ADR-0155）。
_Avoid_: thread-local cache、全renderer共有cache、Platform Frontによるbudget所有、Core source resourceとの同一視
...
Web の既定 `Scene Renderer` 試行順。WebGPU が使えるときは Vello を第一候補、初期化失敗時の fallback に tiny-skia を置く。WebGPU が使えないときは tiny-skia を直接選ぶ。共通契約エラーは fallback の理由にせず、その入力を失敗として扱う。
...
skia-safe の初回初期化に成功した後に発生する描画・surface・context エラー。選択済み renderer の状態を捨てて別 renderer を起動する runtime fallback / restart は行わず、App Host に terminal failure として報告する。初回 boot 中の候補試行や他 renderer の既存 fallback 方針とは別の状態である。
_Avoid_: 選択後の自動 backend 切替、context loss の透過的復旧、初回 init failure との同一視

## Runtime Boundaries

**App Host（アプリホスト）**:
プラットフォーム非依存の最上位協調層であり、mount 先。`ElementTree` 実体の所有・フレームループ・`Event Delivery` の drain・Font ロード・native surface ごとの `Native Accessibility Session` を担い、内部で **Render Host**（描画オーケストレーション・ADR-0068）を駆動する。OS フレームループは所有せず、`tick(timestamp_ms)` を公開して Platform Front から毎フレーム呼ばれる。フレームを起こす入口は単一の `request_redraw`（= wake）で、継続（進行中 animation / `visual_dirty`）は App Host、入力到着は Platform Adapter、非同期 signal 変化（Resource / Store / timer）は consumer が同じ入口を叩く。frame の意味順序は accessibility action drain → delivery drain → consumer flush → commit/present → accessibility outbound update で、App Host 自身が `Idle` / `Prepared(frame_id)` の状態機械として順序を保証する。in-process projection は二相を単一 `tick(timestamp_ms)` 内で連続実行し、wire projection は Rust の可変所有を JS callback へ再入させず二相間で制御を返す（ADR-0150）。pending が無ければ idle に落ちる。Hayabusa（in-process Rust）と Tsubame Hayate Renderer（wire 経路）は共にこの App Host へ mount し、tree/loop/drain/描画/font/accessibility の意味を自前で再定義しない。Platform Adapter は App Host の裏に薄い trait（`Surface` / `FontFetcher` ＋ IME・入力・native accessibility target）として収まり、Hayabusa は Platform Adapter も Platform Front も直接触らない。DOM input、Canvas resize、IME、clipboard、adaptive render scale など platform 固有の前後処理は Platform Adapter の深い frame façade に集約し、App Host に持ち込まない。ADR-0068 の共有層を実体化・拡張したもの。
...
Core の frame commit が返す renderer-ready かつ platform-free な不変 value。`Scene Snapshot`、`Layer Topology`、content/chrome dirty layer、scroll layer の compositor 入力、pending visual work の有無を一括で所有する。App Host はこれだけを `PresentTarget` へ渡し、Render Host に `ElementTree` を公開しない。background、surface size、render scale と backend 用 scroll geometry は platform/renderer projection が補う（ADR-0150、ADR-0153）。
_Avoid_: `Scene Snapshot` だけを完全な frame output とみなす理解、mutable SceneGraphまたは`ElementTree`全体をrendererへ渡す設計、DOM/Canvas固有値を含める設計

**Layer Topology（レイヤートポロジー）**:
Core commit が確定する layer の描画順・親子関係と、その frame で構造または placement geometry が変化した layer 集合。実際の placement、抽出済み sub-scene、renderer 固有資源は含まない。
_Avoid_: SceneGraph 全体、Layer Presentation の retained state、renderer 側で全 SceneGraph を比較して再発見する設計

**Layer Presentation（レイヤープレゼンテーション）**:
`Committed Frame` を layer 単位の描画 transaction へ投影する renderer-neutral な presentation model。Core が確定した frame facts と `Scene Renderer` 固有資源の間に位置し、前回成功した presentation state を正本とする（ADR-0152）。
...
**Native Accessibility Session**:
native surface ごとに App Host が一つ所有する、Desktop・Android・iOS 共通の accessibility lifecycle module。AccessKit callback が積む activation / action mailbox を frame 前半で drain し、Core commit 後の確定した layout・focus から initial / subsequent `TreeUpdate` を生成して、mount 済みの leaf native accessibility target へ渡す。`Detached / Dormant / InitialPending / Active` 状態機械と送信済み `NodeId → Node` baseline を持ち、変更 frame では全 accessibility tree を構築して値比較するが、target へ送るのは新規・変更 node だけとする（MVP。dirty subtree からの部分 walk は後続）。Core の logical bounds に Platform Adapter 由来の base content scale を root transform として適用し、adaptive render scale や screen-space origin は持たない。callback thread から `ElementTree` を借用せず、action → delivery → consumer mutation → layout → accessibility outbound の順序と wake を一箇所で保証する。Web の読み取り専用 `Accessibility Mirror` は別 projection であり session に含めない（ADR-0158）。
_Avoid_: AccessKit の platform adapter 自体、Platform Front が `tick` 前後で手動駆動する lifecycle、thread-safe な `ElementTree` snapshot、Accessibility が interaction state を所有する理解
