...
Hayate-Tsubame 間の現行正本は [`proto/spec/`](../../proto/spec/) の JSON 群（npm: `@hayate/protocol-spec`）である。`apply_mutations(ops: Float64Array, styles: Float32Array, texts: string[])` と `poll_events` の定数・種別・フィールド名はこの spec に従う。
