...
Raw Layer は現時点では WIT world から export されない。インターフェース定義は `wit/hayate.wit` に存在するが、全ノード型の実装と具体的なユースケースが揃うまで非公開とする（ADR-0033）。
...
| `element-set-transform(id, matrix)` | 2D アフィン変換行列を設定（`record affine`） |
...
| `element-get-bounds(id)` | レイアウト後の座標・サイズを `record bounds` で取得（ADR-0031） |

**入力**

| 操作 | 説明 |
|---|---|
| `element-paste(id, text)` | ペーストテキストを text-input 要素に渡す（ADR-0031） |
| `element-load-font(data)` | カスタムフォントをバイト列でフォントレジストリに登録（ADR-0028） |

**フレームライフサイクル**

| 操作 | 説明 |
|---|---|
| `render(timestamp-ms)` | フレームを描画。単調増加タイムスタンプをカーソル点滅に使用（ADR-0032） |
| `poll-events()` | イベントキューを取得（下記参照） |

### 3.7 Interaction Events（ADR-0019、ADR-0031）
...
イベント命名は**セマンティック型**を採用する（物理操作名ではなく UI 状態遷移名）。既存の `click` / `focus` / `blur` / `scroll` と命名スタイルを統一する（ADR-0031）。

**ポインタ・インタラクション**

| イベント種別 | ペイロード | 意味 |
|---|---|---|
| `click` | `{ target, x, y }` | クリック確定 |
| `hover-enter` | `element-id` | ポインタが要素に入った |
| `hover-leave` | `element-id` | ポインタが要素を離れた |
| `active-start` | `element-id` | ポインタ押下（`:active` 状態開始） |
| `active-end` | `element-id` | ポインタ離放（`:active` 状態終了、座標なし） |
| `pointer-move` | `{ x, y }` | ポインタ移動（target なし、キャンバス絶対座標） |

**キーボード**

| イベント種別 | ペイロード | 意味 |
|---|---|---|
| `key-down` | `{ target, key: string, modifiers: flags }` | キー押下。`key` は Web KeyboardEvent.key 準拠 |

`modifiers` は WIT `flags modifier-keys { shift, ctrl, alt, meta }` 型。
...
| イベント種別 | ペイロード | 意味 |
|---|---|---|
| `text-input` | `{ target, text }` | 確定テキスト入力 |
| `composition-start` | `{ target, text }` | IME 変換開始 |
| `composition-update` | `{ target, text }` | IME 変換中 |
| `composition-end` | `{ target, text }` | IME 変換確定 |

**ビューポート**

| イベント種別 | ペイロード | 意味 |
|---|---|---|
| `scroll` | `{ target, delta-x, delta-y }` | スクロール delta |
| `resize` | `{ width, height }` | ビューポートサイズ変更 |
