...
移行中は既存のsub-scene copy経路をtest oracleとしてだけ使い、同じfixtureのDrawOp、placement、pixel結果をzero-copy Layer Sceneと比較する。全Scene Rendererのparityが成立した時点で旧抽出関数、旧interface、runtime flag、fallback、旧implementationを直接検査するtestを完全削除する。旧・新scene表現を永久に併存させる設計は採らない。
