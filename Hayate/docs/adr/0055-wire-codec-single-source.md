...
ADR-0049 はプロトコル**定数**の単一正本を導入した。`apply_mutations` については Rust 側 decode（`parse_next_op` / `decode_style_packet`）のみ生成され、TS 側 encode（HayateRenderer の semantic queue / `style-codec.ts`）は手書きのまま残っていた。event 方向（Rust encode / TS decode）は既に対称。mutation/style の op セマンティクスが二重管理になっている。
...
- **spec が所有しないもの**: `StylePatch` / HayateRenderer 所有の semantic queue（Renderer Protocol。Contract 外）
...
手書き `style-codec.ts` / `style-encoder.ts` shim は**削除**。HayateRenderer 所有の semantic queue は flush 時に generated codec を呼ぶ。
...
- HayateRenderer 所有 semantic queue の spec 生成
...
4. `@torimi/tsubame-protocol-generated` exports に `./codec` 追加。

### Task 4 — renderer-canvas 移行

1. `style-codec.ts` / `style-encoder.ts` 削除。
2. HayateRenderer 所有の semantic queue の flush を generated `codec.ts` 呼び出しに差し替え。
...
4. **C3**: 既存 wasm / renderer-canvas 結合テストを generated codec 経由に更新。✅ `wasm-integration.test.ts` + `pkg-null`（`backend-null`）。

### Task 6 — CI

1. `npm run check:proto` / `pnpm run check:proto` に codec.ts / codec.rs diff を含める。
2. C3 用 wasm ジョブ（全体 PR または wasm 変更時）。✅ `.github/workflows/wasm-c3.yml`。
