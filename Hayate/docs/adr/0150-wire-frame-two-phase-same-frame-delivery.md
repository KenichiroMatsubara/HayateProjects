...
Core の frame commit は renderer-ready かつ platform-free な不変 value `CommittedFrame` を返す。これは owned `Scene Snapshot`、Layer Topology、content/chrome dirty layer、scroll layer の compositor 入力、pending visual work の有無だけをまとめる。Layer Topology は layer の描画順・親子関係と、構造または placement geometry が変化した layer 集合を Core が commit 時に確定した frame fact であり、実際の placement や renderer 資源は含めない。Scene Snapshot は後続 lowering と独立した寿命を持ち、Raster Thread への handoff は snapshot を O(1) で共有する（ADR-0153）。App Host は `PresentTarget::present(&CommittedFrame) -> Result<(), FrameExecutionError>` を呼び、mutable `SceneGraph` と `ElementTree` を Render Host へ公開しない。Web の layer compositor は `CommittedFrame` の scroll 入力を backend 用 geometry へ変換し、background、surface size、render scale など platform 固有値は PresentTarget 側で補う。
