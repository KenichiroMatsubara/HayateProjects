...
Hayate には Hayabusa と Tsubame という二つの文脈がある。Hayabusa は ADR-0045 により Rust crate 依存の長期構想として整理された。一方で短期的に契約整備が必要なのは、Hayate-Tsubame 間の `protocol.yaml`・`apply_mutations`・`poll_events` である。文書上は依然として Hayabusa 中心、WIT 中心の説明が多く、現行の実装優先度を誤解しやすい。
...
- 現行の実装契約と文書の重心が一致する。
- `protocol.yaml` 正本化以降の説明が読みやすくなる。
