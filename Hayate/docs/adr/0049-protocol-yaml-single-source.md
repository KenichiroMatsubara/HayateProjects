...
> **2026-06-06 追記（ADR-0053）:** 正本形式は当初の単体 `protocol.yaml` から `Hayate/proto/spec/*.json` + JSON Schema + npm パッケージ `@hayate/protocol-spec` へ移行済み。本 ADR の「単一正本」原則は維持し、形式のみ更新する。
...
配布: npm パッケージ `@hayate/protocol-spec`。Tsubame は正本を持たず workspace 依存として取り込む。
...
**Scope（[ADR-0055](0055-wire-codec-single-source.md) で拡張）:** 定数に加え **wire codec**（`apply_mutations` の encode/decode）を spec から生成する。semantic 層（`StylePatch` / `HayateMutationPacket`）は Contract 外のまま。
