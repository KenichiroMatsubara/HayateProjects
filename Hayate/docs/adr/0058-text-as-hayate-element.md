# Solid の text は Hayate text element として Canonical Tree に載せる
...
- ADR-0057 の Host 側は `ElementId` ハンドル列のみ（text も element）。Canonical Tree 正本は Hayate（Canvas）または DOM（DOM Renderer）
