...
- **派生物は2つ・dirty 集合は2つ**（ADR-0063 と合わせた対称性）：`ElementTree`（唯一 owner）から **Taffy projection**（`structure_dirty` で reconcile、block 構造変化のみ）と **InlineText layouts**（`shape_dirty` で再 compose、IFC 内容/スタイル変化）が派生する。inline span の mutation は `shape_dirty` のみで `structure_dirty` に触れない（span は Taffy ノードを持たない）。Taffy `mark_dirty`（layout-dirty）が両者の橋渡し：構造変化も shape 変化も影響 node を mark_dirty して geometry を再計算させる。
