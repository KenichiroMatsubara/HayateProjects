...
- element 位相・layout 幾何・per-element `EditState`・scroll offset は所有せず、private
  `InteractionSession` が `ElementTree` の排他的借用内で document 能力と interaction state を扱う。
  Event と dirty mark もこの session 内で実 tree に適用する。
- **interface = test surface**：公開入口は `ElementTree::apply_interaction_intent` だけで、実 tree に
  intent を流して状態と発火 Event を検証する。fake view と public trait は置かない。ADR-0066 が
  runtime 所有とした interaction 状態の所在を、この module へ精緻化する（runtime 内の再配置）。
...
- **`EditState`**：`edit_state.rs`（1109 行）は `EditIntent` を純粋適用し単体テスト済みで、型として既に deep。
  ...
  storage は `Element.edit` のまま、private session が実 tree から借りる。
...
- `interaction.rs` の interaction 状態機械は private `InteractionSession` に集まり、
  `ElementTree::apply_interaction_intent` の実 tree surface で intent を検証できる。今テスト 0 の最大領域に
  regression surface が付く。
