...
clean frame は Layer Scene の生成も scene walk も行わず、scroll affine だけの変更は placement だけを更新する。dirty・追加・構造変更された layer は zero-copy Layer Scene を一度だけ生成し、canonical walk を一度だけ行う。execute が失敗した場合は retained revision と cache ledger のどちらも更新せず、未反映 dirty workを失わないが、Layer Presentation自身はretryをscheduleしない。削除された layer の retained state と backend resource は transaction が成功した commit 時にだけ破棄する。
...
placement の更新範囲は Committed Frame の Layer Topology から決め、Layer Presentation が SceneGraph 全体を diff して構造変更を再発見する設計は採らない。content だけの変更では placement を再計算せず、layer 自身の transform または祖先 clip の変更は影響する layer subtree だけを更新する。layer の昇格・降格・並べ替えも Layer Topology が示す影響範囲だけを再構成し、scroll affine は既存の compositor 入力から該当 placement だけへ反映する。実際の placement と適用済み revision は Layer Presentation が retained state として所有する。
