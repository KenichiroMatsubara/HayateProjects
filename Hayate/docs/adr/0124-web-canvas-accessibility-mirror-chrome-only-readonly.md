...
- **Tsubame 側（renderer-hayate 等）に置く** — ARIA ブリッジはレンダラの責務でなく、ADR-0001 の Hayate↔Tsubame 境界を汚す。却下。`@torimi/hayate-host` なら Hayate 側に閉じつつ canvas+raw を既に握っている。
...
- `@torimi/hayate-host` に置くため、**全 Canvas アプリが host boot 経由で自動的に**テスト可能な ARIA 面を得る（host-boot 毎の配線が不要）。
