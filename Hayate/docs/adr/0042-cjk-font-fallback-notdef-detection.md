...
> **更新（ADR-0106 / issue #343）**：この HashSet は `FontFetchTracker` に置き換えられた。削除が成功時のみだったため、フェッチ失敗時にファミリが恒久的にラッチされ再要求されない不具合があった。現在は失敗報告経路（`font_fetch_failed`）＋有限リトライ予算＋アダプタ側バックオフで、失敗しても後続フレームで再要求される。
