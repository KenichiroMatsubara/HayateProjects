...
> **Superseded by ADR-0049** — OP_* 定数は `@torimi/hayate-protocol-spec`（`proto/spec/*.json`）が単一正本となり、generator で生成される。
...
> op テーブルに OP_CREATE=9（slots=3: `op, id, kind_code`）を追加すること。
...
- **~~`element_create`~~**: ~~戻り値（ElementId）が必要なため。呼び出し側はフレーム開始前に個別呼び出しで ID を取得し、その ID を ops ストリームで参照する~~ → **廃止（Tsubame ADR-0005）。`OP_CREATE=9` として ops バッチに含める。**
