...
**前提タスク**: `element_effective_visual` は現在 `ElementTree` の public メソッドだが wasm bindings（`platform/web/src/element_renderer.rs`）から export されていない。golden frame 取得のため export を追加する。
