...
> 2026-07-20: resource budgetの所有はADR-0155で改定した。Platform Frontはbudgetを持たず、platform capabilityとmemory pressureを材料にRender Hostがbudget policyを決め、選択済みScene RendererのRender Resource Residencyへ適用する。
...
- **予算の所在**: platformのcomposition rootはdevice memory class、surface size、OS memory pressureをRender Hostへ渡す。Render HostがCPU/GPU別のbudget policyを決め、選択済みScene RendererのRender Resource Residencyへ適用する。Platform Frontはschedule専任でbudgetを持たない。コア（ADR-0125）のレイヤ判定・`layer_dirty` は予算を知らず不変に保つ。
...
- **予算をPlatform Frontに持たせる**: frame schedulingとresource lifetimeを混在させるため却下（ADR-0155）。
...
- ADR-0155（Render Resource Residency）がlayer textureを含むrenderer資源全体の所有とbudget policyを定める。
