...
- :hover 等のインタラクション状態スタイル（ADR-0019 — Framework Signal 責務）
...
| `host-echo` | resize（wire） | 届けない。`IRenderer.resize` 指令が正 |
...
配布: npm パッケージ `@hayate/protocol-spec`。Tsubame は正本を持たず依存として取り込む。HayateProjects モノレポは AI/クラウド作業都合であり、アーキテクチャ上の結合点ではない。
...
Tsubame 生成射程は **wire + adapter vocabulary**（`StylePatch`, `EventKind`, semantic mutation surface, delivery wire 型）。`IRenderer` の tree/style/imperative メソッド型は generated vocabulary の thin wrapper。`setProperty` / `addEventListener` 購読 API / `resize` は Contract 外。
...
- `@hayate/protocol-spec` npm パッケージ新設
