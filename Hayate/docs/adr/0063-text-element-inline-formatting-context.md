...
- **Taffy 構築**: `is_inline(id)`（`text` かつ祖先に IFC root）なら Taffy ノードを作らない。IFC root は measure 関数で subtree を整形する leaf。この span 除外・IFC-leaf 化は **Taffy 投影の責務（ADR-0064 の `TaffyProjection` が lazy reconcile で実施）**。`el.taffy_node: Option`（span は None）。
- **dirty 伝播**: inline span への `setText` / `setStyle` / append / remove は span ではなく **IFC root を再 shape 対象**にする（span に layout が無い）。mutation 経路で `ifc_root(id)`（親が `text` である限り遡上、O(inline 深さ)）を求め、**`shape_dirty.insert(root)` ＋ Taffy `mark_dirty(root.taffy_node)`**。**inline 変化は Taffy 構造を変えないので `structure_dirty`（ADR-0064）には触れない**。`run()` で shape_dirty の IFC root を InlineText で再 compose（プリパス）→ measure closure が line-break。（color/decoration は geometry 不変だが、初期実装は常に再 measure で単純化し、metric 非影響プロパティのスキップは後続最適化。）
