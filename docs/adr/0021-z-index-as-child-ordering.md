...
- HTML Mode では要素に `z-index` CSS プロパティを直接設定する。全要素が単一コンテナ直下の `position: absolute` であるため CSS stacking context は一つだけであり、整数値による描画順序制御は Canvas Mode の painter's algorithm と等価になる（詳細は ADR-0029）
