...
- 委譲先: todo/react-todo の `build:android`→`torimi:native:build`、`build:torimi`→`torimi:web:build`。todo の `torimi:android:serve`→`torimi:native:dev`、`torimi:web:dev` 新設。Hayate の `desktop`／`desktop:build`→`hayate:desktop:run`／`hayate:desktop:build`。
