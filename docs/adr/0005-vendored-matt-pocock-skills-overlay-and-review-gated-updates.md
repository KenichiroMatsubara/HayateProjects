...
独自編集を持つスキルには、同ディレクトリに **`OVERRIDES.md`**（このリポジトリ独自の追加・
変更を記録するファイル）を置く。SKILL.md の冒頭には標準の**参照ブロック**を入れ、「本文を
読む前に `OVERRIDES.md` を読み、**衝突時は `OVERRIDES.md` を優先**せよ」と明記する。これに
より、何が上流由来で何がリポジトリ独自かが一目で分かり、更新時に独自部分を保護できる。

ユーザが新たに独自仕様を足すときも `OVERRIDES.md` に書く。**その追加が元スキルの設計思想と
...
語の変更）→ 危険印を付けて提示 → **更新1回ごとにユーザの最終 OK で適用**。`OVERRIDES.md` と
...
- **良い点：** 更新が人間のレビュー関門を必ず通る。独自チューニングが更新で消えない。何が
  独自編集かが `OVERRIDES.md` に明示され、将来の読み手にも追跡可能。pin により再現性がある。
- **コスト：** チューニング済みスキル（`pinnedLocal`）は上流本体を自動追従しないため、上流の
  ...
  受容するリスクであり、`pinnedLocal` と OVERRIDES の明示で被害面を最小化する。
