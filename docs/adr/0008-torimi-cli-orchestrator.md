...
5. **vite preset は 2 部品に分解して配る**: FW 変換は各 adapter の subpath（`@tsubame/solid/vite` / `@tsubame/react/vite`）、App Bundle 形状（単一 IIFE・es2020・DOM なし・非圧縮）は `@torimi/bundle/vite`。CLI 本体には置かない（ビルドツール非依存を維持）。
