...
[PREEDIT-INK]     変換前 underlines=[1.0]／変換中 underlines=[2.0,1.0] ← #336 修正済み（clause 下線）
```
...
### 5.【Medium → 修正済み #337】1px ボーダーが全く描かれない（ラスタ課題ではなかった）

> 当初「整数座標ヘアラインが AA で塗りに飲まれる**ラスタ課題**」と推測したが、
> `/diagnose`（`DrawOp` 記録 + 列走査）で根因は**全く別**と判明した。tiny-skia /
> vello いずれの**ラスタライズの問題でもなかった**。

**根因 A（共有・全レンダラー）**: todo アプリの各スタイル（`inputStyle` /
パネル / ボタン / セグメント）は `border-width` と `border-color` を指定するが
**`border-style` を指定していない**。#204 で border-style は CSS 同様 `none`
が既定（width だけでは solid にしない）と決めたため、**ボーダーノードが 1 つも
emit されない** → tiny-skia・vello・DOM の**いずれでも描かれない**。`DrawOp` を
記録すると入力欄に対し `FillRect`（panel2 塗り）が 1 つだけで、ボーダーの outer/
inner は存在しなかった。`border-style: solid` を補うと、整数 x の 1px ボーダーは
**そのまま鮮明な独立不透明列**として乗る（ラスタ補正は不要だった）。
→ 修正: `theme.ts` / `App.tsx` の各スタイルに `borderStyle: 'solid'` を追加。

**根因 B（tiny-skia 固有・#335 リグレッション）**: `:focus-visible` のネイティブ
focus ring（#335）は box の上に重ねて描く `RoundedRing` だが、tiny-skia の
`fill_rounded_ring` は内側を **`BlendMode::Clear` でくり抜いて**いた。これは下の
不透明コンテンツ（focus 中の入力欄の塗り・accent ボーダー・テキスト）まで**透明に
消去**し、focus すると入力欄が穴になっていた（`[FOC-CENTER]` が `(0,0,0,0)`）。
vello は outer−inner の even-odd バンド塗りで下を消さないため正しく、tiny-skia
だけがバグっていた。→ 修正: tiny-skia も even-odd バンド塗りに統一（Clear 廃止）。

両修正後、入力欄左ボーダーは unfocused で `line`、focused で `accent`(teal) が
独立列として検出され、focus 中の入力欄内部も不透明を維持する（回帰テスト
`addform_input_1px_border_renders` / `border_raster_regressions_{tiny_skia,vello}`）。
...
### 7.【High → 修正済み #336】IME preedit が素のテキストで描かれていた（変換中の下線が出ない）

**かつては** 変換中（committed「」＋ preedit「ぎゅうにゅう」）を Canvas は
`display_text` ＝ `content + preedit` を**一続きの素のテキスト**として描き、preedit
区間に装飾を一切付けなかった（`display_text` は連結のみ、`scene_build` は単一 run）。
DOM / EditContext は composition を**下線**で示すため「どこが未確定か」が Canvas では
分からなかった。

**現在は** ADR-0102 に従い、EditContext `textformatupdate` の clause 範囲を
JS→wire→core に配管し、`EditState::preedit` を範囲付き（`Preedit { text, clauses }`）へ
拡張。`scene_build` が clause ごとに下線 rect を emit する — 変換前は 1 本の細下線、
変換中は変換対象 clause が太線・確定 clause が細線（`[PREEDIT-INK]` に
`変換前=[1.0]／変換中=[2.0,1.0]` として現れる）。下線の実 px 太さは実 Chromium で校正予定。
...
- **#5 focus**：HTML Mode の `outline: none`（`html.rs`）は本原則違反の**バグ→除去**。
  ...
  1px ボーダーが「薄い」件は**ラスタ課題ではなく**、アプリが `border-style` 未指定
  （#204 で既定 `none`）→ 全レンダラーで未描画だったのが根因。`border-style: solid`
  補完 + tiny-skia の focus ring Clear 廃止で解決（#337、上記 finding #5）。
- **#6 selection**：highlight tint は browser `::selection` に寄せる。handle / ツールバー /
  拡大鏡はブラウザに無い概念なので Android-native（Selection Chrome 改定）。
- **#7 preedit → 修正済み #336**：EditContext `textformatupdate` を配管し、clause 分割
  下線を Chromium 同様に再現（`preedit` を範囲付きへ拡張）。
