...
**修正案（分析のみ・未実施）**:
- `core/.../text.rs::codepoint_font_family` に emoji 範囲
  (`0x2600..=0x27BF`, `0x1F300..=0x1F5FF`, `0x1F600..=0x1F64F`, `0x1F680..=0x1F6FF`,
  `0x1F900..=0x1F9FF`, `0x1FA70..=0x1FAFF` など) の arm を追加し、tiny-skia が描ける
  **モノクロ** `Noto Emoji` を返す。
- `adapters/web/fonts.json` に `Noto Emoji`(`ofl/notoemoji/NotoEmoji[wght].ttf`) の URL を
  追加して `builtin_font_url` 経由の DL 経路を成立させる。
- 注意: `Noto Color Emoji`(COLR/CBDT) は tiny-skia の outline-only painter では描けない。

> 上記は分析に基づく提案であり、本タスク（分析）では**コード変更は行わない**。
> 実装する場合は別途指示が必要。視覚的 end-to-end 確認には CDN への egress（要
> ネットワーク / CI）が必要で、本実行環境では未検証。
