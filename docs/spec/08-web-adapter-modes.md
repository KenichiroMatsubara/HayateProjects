...
**備考:** ★ 規範文の「いずれか欠ければ HTML Mode」を満たすには `@torimi/hayate-host` の backend resolve に EditContext probe + HTML renderer 経路を追加する必要がある（実装ギャップ、徹底実装フェーズの作業対象）。判定の所在は Tsubame init ではなく Hayate web host bootstrap（ADR-0004 で確定）。[追加 2026-07-12] Canvas backend（vello ↔ tiny-skia ↔ vello-cpu）のランタイム切替は、APK を作り直さず intent extra で切る Android（`hayate.renderer`, REND-15）と同じ操作感を web にも通す。値語彙は `SceneRendererKind::name()` と同一の `?renderer=vello|tiny-skia|vello-cpu`（named constant `RENDERER_QUERY_PARAM`, `resolve-backend.ts`）。`createHayateWebHost` 自体が明示 override＞クエリ強制（deep-link）＞WebGPU 自動判定の順で解決し（`resolveCanvasBackendSelection`）、選択 backend と理由（`options-override`/`query-override`/`webgpu-auto`/`webgpu-unavailable-fallback`）を console に記録する。デモページ（`examples/todo/index.html` の `#renderer-switch`）がタップで `?renderer=` を書いてリロードする観測面を提供。Chrome 実機（WebGPU/AMD RDNA-2）で vello 描画・tiny-skia フォールバック・強制切替を live 確認済み。[追加 2026-07-12] Web デモの既定レンダラは `skia-safe`（プロジェクト主力）に変更。ただし rust-skia は wasm32 非対応でブラウザでは動かせない（ADR-0146）ため、`skia-safe` は Web ではラベルを保ちつつ skia 系 CPU ラスタライザの tiny-skia backend へ委譲する（`detect-mode.ts` の `DEFAULT_RENDERER`/`SKIA_SAFE_WEB_BACKEND`、host 側 deep-link は `resolve-backend.ts` の `RENDERER_VALUE_SKIA_SAFE`）。あわせて `#renderer-switch` の「最適化(layer-present)」行は、選択レンダラに対応する 1 行だけを表示するよう修正（vello 行と tiny-skia/vello-cpu 行が常時二重表示されていたバグの解消）。
