...
**状況:** 🟡部分 — hayate-core: `inline_text.rs` の `shape(ifc_root, width)->(Layout, RangeMap)`、`build_ranged_text_layout`、`shape_dirty` 伝播、measure 経路の IFC 合成整形、二段 hit-test（byte→inline text element）。Taffy inline 除外は LAY-04（ADR-0064）で実装済み。残: `tsubame-solid` collapse 撤去（§11 TSUB-02 / 残タスク10）で shadow `node.text` を閉じる。
