...
| C-8.3 | §8 WEBA-01「probeWebGPU で Canvas/HTML モード自動判定」 vs 実装は GPU/CPU バックエンド選択のみ（HTML renderer は Tsubame init から到達不能） | 2026-07-25 解決。Web entry は明示 DOM / EditContext 欠如だけを HTML Mode へ退避し、Canvas Mode は標準 OffscreenCanvas＋単一 Worker へ cutover。renderer 初回選択は Worker 内 Render Host が所有（WEBA-01 / ADR-0157）。 |
