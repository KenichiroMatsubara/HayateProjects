...
**規範文:** テキスト継承は2チャネル。**(1) 通常 text スタイル**（`color`/`font-size`/`font-family`/`font-weight`/`font-style`/`text-decoration`）は **text→text（IFC 内 inline text 連鎖）のみ**継承し block を貫通しない（`view` に置いても text に漏れない）。`InlineText`（ADR-0063）が解決。**(2) ambient Default Text Style**（`default-color`/`default-font-family`/`default-font-size`/`default-font-weight`）は任意 element に設定でき **block を貫通**して既定値を供給する（Flutter `DefaultTextStyle` 相当）。`scene_build.walk` の top-down 機構が運ぶ。**ambient 既定は self-inclusive** — 要素自身に置いた `default-*` は子孫だけでなく**その要素自身**のテキスト解決にも効く（直感的挙動。DOM Renderer の `default-*`→CSS 継承プロパティ自己適用と一致）。解決順: 自身明示（ch1）→ text 祖先継承（ch1）→ ambient 既定（**自身＋最寄り祖先**・block 貫通）→ ハード既定。プロパティ名は CSS 準拠、適用は Flutter 寄せ（LLM 予測可能性）。
**出典:** ADR-0065（ADR-0047 を supersede）
**状況:** ✅ — `ambient_defaults.rs`（ch2 ambient `default-*`・block 貫通）、`inline_text.rs`（ch1 text→text 継承＋ambient フォールバック）、`scene_build.rs`（`AmbientDefaults` threading）。`effective_visual.rs::apply_text_inheritance` が ambient を self-inclusive に畳む（`ctx.ambient.merge_visual(own)`）。`style_tags.json` に `font-style`/`text-decoration`/`default-*` 追加済み。回帰テスト `text_inheritance.rs`（`own_default_color_applies_to_self_text` 他）、parity は `golden-frame-parity.test.ts`。
...
### ELEM-03 — 単一正本 Canonical Tree（描画正本）
