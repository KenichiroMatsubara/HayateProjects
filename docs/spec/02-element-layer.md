...
**状況:** ⬜未実装 — 設計確定。現状は ADR-0047 の CSS 式（全 element で color/font_size/font_family が継承、`scene_build.rs` の `InheritedStyle`）。2チャネル分離・`InheritedStyle` の ambient 既定転用・新 style prop（`font-style`/`text-decoration`/`default-*`）の spec proto 追加・`font-weight` の text→text 継承化が残タスク。
...
**状況:** ⬜未実装 — 設計確定。現状は `resolve_visual`（`pseudo_state.rs:128`）が `scene_build.rs:64` の walk 内でのみ呼ばれ結果を SceneGraph に emit して捨てる（query 不可・test surface が SceneGraph）。継承 ch2 は `scene_build` で別途（`:74–84`）、ch1 は `InlineText`（ADR-0063）と3分散。shared resolver 抽出・query 露出・3 caller の共有が残タスク。
...
| 状況 | 件数 | ID |
|---|---|---|
| ✅実装済み | 3 | ELEM-01, 03, 05 |
| 🟡部分 | 1 | ELEM-04 |
| ⬜未実装 | 2 | ELEM-02（text 継承の2チャネル化、ADR-0065）, ELEM-06（実効スタイル単一 resolver、ADR-0067） |
